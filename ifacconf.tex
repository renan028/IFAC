...
%% Opção 2 de filiação sem email
 ...
 \address[2]{Department of Mathematical Sciences and Technology, Norwegian
University of Life Sciences, {\AA}s, Norway}
 \address[3]{Research and Development Center, Petrobras/CENPES,
...
rise several challenges for future work.
\end{abstract}
...
vibration and temperature sensors. The robotic device moves on a rail
and both robot and rail are based on a modular concept.
...
%Mechanics
%-------------------------------------------------------------------------------------------------------------------------
The mechanics comprises the modules, the joints used to couple them, and the
rail. The design allows the robot to move smoothly in a 3D space and to make a
full stop anywhere on the track including the vertical segment. It incorporates
the use of gimbals with traction and guide wheels surrounding a tubular rail. The rail is designed to be as simple as possible, leaving the complexity to the robot, which is motivated by the fact that the rail may be extremely long so that the cost of it should be kept to a minimum. The use of two sets of gimbals provide mechanical compliance with rail curvatures, smoothness of the robot's base movement, and good weight distribution.
...
designed to be augmented with additional modules. The total weight of this basic
configuration is estimated at 50 kg and DORIS can reach a maximum speed of 1m/s.
...
power consumption. Further complications arise because the system is designed to
move in confined areas and have efficient wireless communication with operators, providing
...
DORIS traction is taken care of by the \emph{actuation system}, which is
composed of four controller drivers, and four motor packs, each containing a high power 200 W EC-4pole Maxon brushless motor, an encoder and a planetary gearhead.
...
The BMS communicates with the batteries via \emph{System Management Bus}
(SMBUS), and it has a connection with the BMS bus switching board. The SMBUS collects important information from the batteries, such as temperature, voltage, current and remaining charge. The BMS bus switching system contains high power solid-state relays (20 A), which can be commanded from the BMS AVR to distribute the power to
...
Concerning the electronics, power supply, and software, the first objectives of
SAM were to test the following concepts:
\begin{enumerate}[i)]
...
\begin{enumerate}[i)]
  ...
  during the downhill motion. To prevent this, a shunt regulator
  ...
  big capacitor bank could be used as an energy storage device. Tests will be
  performed to measure the generated amount of energy to decide if it is worth
  to embed a device to store or waste it;\\
