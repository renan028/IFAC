...
%%% Opï¿½ï¿½o 1 de filiaï¿½ï¿½o com email
...
%% Opï¿½ï¿½o 2 de filiaï¿½ï¿½o sem email
  \address[1]{Electrical
...
through rail carrying different sensors, and
...
and to make a full stop anywhere on the track. It incorporates the use of gimbals with
containing traction and guide wheels which surrounds a tubular rail. Since the
rail in an offshore facility may be extremely long, it
is designed to be as simple as possible to keep its cost to a minimum, the
...
The computer is basically composed of a high performance Intel\textregistered
...
The remote operation base is the user workstation in the offshore facility
through where DORIS can be remotely operated. It is composed of a radio
...
traction is commanded via CAN bus (computer to driver), which provides
reliability, safe transmission and the appropriate speed to this application
...
The \emph{data acquisition system} collects image, video and audio from
the environment. It is composed of a fixed camera, an infrared thermal camera
for thermal map and temperature measurement, a \emph{fisheye} camera, and two
stereoscopic webcams (with embedded microphone).
...
and state of charge. Also, the \emph{BMS PCB}' AVRs report all data periodically
to DORIS computer via Ethernet with the UART-to-Ethernet converter. 
...
heating. Each battery pack has a capacity of 10 Ah when deliveing 24 Vdc.
...
Batteries are connected to the \emph{PBS PCB}, which has solid
...
libraries), and also the \emph{ROS node} \emph{Robot GUI} (load those
...
related to video, audio, data table, gamepad, devices configuration that can be
used on other robotic systems. The \emph{DORIS Package} is more specific to
DORIS and deals with its hardware and functionalities: it has just one
\emph{Tool} to control the 4 motors; the \emph{Components} acquire video from
an IP camera, interface with the CAN bus to control the motors, communicate
with the PCB boards through a serial bus, and so on.
