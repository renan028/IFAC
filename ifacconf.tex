...
cost and rise the employee salary. In order to be competitive and to improve
their profit, oil \& gas companies are looking for new technologies to reduce
the labor costs. The use of robotics in inspection maintenance and repair
operations in oil \& gas facilities could greatly improve productivy,
efficiency, health and safety, while decreasing operational and logistics
costs.
...
based on advanced robotics systems (\cite{presal}).

The work conditions on offshore installations such as unfriendly atmosphere,
unsheltered environment, heavy wheather, extreme temperatures, constrained
space, and the logistical issues are serious financial obstacles for oil \& gas
companies. It is highly expensive to have people working on the rig, as they
...
in efficiency and productivity with robot operators. Besides the improvement in
...
\& gas industry and list the most important requirements for robotic systems
are:
\begin{itemize}
...
underwater pipeline repair robotic systems, robots for inpection of valve and
...
descriptions of the embedded electronics, the vehicle support system, and power
supply system are taken in
Sections \ref{sec:electronics_overview},
\ref{sec:VSS}, and \ref{sec:powersupply_overview} respectively.
...
% ESCREVER mais sobre a figura manip com sensor de vib inspecionando equip
...
supply, software, mechanics and signal processing. The mechanical and signal
processing systems were detailed in \cite{OTC} and
\cite{cba}.
...
%Mechanics
%-------------------------------------------------------------------------------------------------------------------------
The mechanics designs the rail, the modules and the joints used to couple them.
The design allows the robot to move smoothly in a 3D space and make a full stop
anywhere on the rail. Considering the severe corrosion and weather conditions in
...
the module on the rail. The use of gimbals is proper choice
concerning complacency in rail curvatures.
...
for communication of control commands; iii) wireless technologies between the robot and the remote control base located at the offshore facility. DORIS can be remotely operated from this base via Wi-Fi IEEE 802.11n or via 2.4/5.0 GHz radio links (upon Wi-Fi failures).
...
application. Since the traction system generates a significant amount of
conductive noise, a galvanic opto-isolator is used between the CAN and the
computer to minimize interference on the rest of the EE system.
%Ref do CAN
%Falar do Watchdog do alex
...
microcontroller based \emph{printed circuit boards} (PCBs) for: failure detection; device protection;
...
and voltage supply, and for protecting them. It is composed of AVR
micrcontrollers, solid-state relays (max. 1.5 A), hall effect sensors (for
...
situations. Since Ethernet is not an available interface in the AT90CAN AVR
...
remaining charge.
% Verificar se é mesmo enrolação
...
efficient electric power distribution for DORIS parts. The power is supplied by high density energy level military lithium ion battery technology, which comes with
an intrinsically safety circuit for protection against short-circuits and
...
%Verificar comparação de baterias se vale ou nao a pena

%Não vale para robos de laboratorio, verificar referencia
As Verma \emph{et al.} (2004) highlights, robots often
...
The power supply architecture of the power supply embedded on this project is
...
%verificar referencia para banco do LUMA
...
Ampere. In our case, as each motor may require a peak current of 20 A, a
...
bank at around 50\%, ie, 35 V the battery voltage is 24 V. In
this case, one must use electrolytic capacitors, other types of
capacitors are not able to provide this capacitance value at this voltage.
The capacitor should have low ESR and it should be located as
close to the noise source, namely the motors.

\begin{figure*}[ht] 
\centering
\includegraphics[width=1\columnwidth]{figs/DiagramaSAM.jpg}  % width is 7.6 cm.
\caption{DORIS power Supply Architecture.}\vspace{-0.25cm}
...
tests of all concepts above. In addition, some VSS functionalities were individually tested, but not yet fully integrated with SAM. The two subsections below detail the tests.
...
The first objectives of SAM test the following concepts:
\begin{itemize}
...
the fall height and the robot velocity, this energy may reach a voltage level
that causes the motor drivers to reset.
% Melhorar explicação do problema
...
prototype) and a RS485 interface with a laptop to emulate the USART
...
Future include: replace RS485 by USART to allow communication with
...
distribution. Additionally, timers will be implemented to : robot
...
\begin{enumerate}[i)]
  ...
  (IMU). The sensor's fusion will precise the Navigation
  System, which estimates the vehicle pose (position, orientation and velocity).
  %Melhorar isso ai
  ...
  order to decide if it would be worth to embeb a device to recover this
